...
func (ct *Peer) Detail(c *gin.Context) {
	...
	u := ct.HD.Services.PeerService.InfoByRowId(uint(iid))
	if u.RowId > 0 {
...
func (ct *Peer) Create(c *gin.Context) {
	...
	p := f.ToPeer()
	err := ct.HD.Services.PeerService.Create(p)
	if err != nil {
...
func (ct *Peer) List(c *gin.Context) {
	...
	res := ct.HD.Services.PeerService.List(query.Page, query.PageSize, func(tx *gorm.DB) {
		if query.TimeAgo > 0 {
...
func (ct *Peer) Update(c *gin.Context) {
	...
	u := f.ToPeer()
	err := ct.HD.Services.PeerService.Update(u)
	if err != nil {
		response.Fail(c, 101, response.TranslateMsg(c, "OperationFailed")+err.Error())
		return
	}
...
func (ct *Peer) Delete(c *gin.Context) {
	...
	u := ct.HD.Services.PeerService.InfoByRowId(f.RowId)
	if u.RowId > 0 {
		err := ct.HD.Services.PeerService.Delete(u)
		if err == nil {
			response.Success(c, nil)
			return
		}
...
func (ct *Peer) BatchDelete(c *gin.Context) {
	...
	err := ct.HD.Services.PeerService.BatchDelete(f.RowIds)
	if err != nil {
		response.Fail(c, 101, response.TranslateMsg(c, "OperationFailed")+err.Error())
		return
	}
...
func (ct *Peer) SimpleData(c *gin.Context) {
	...
	res := ct.HD.Services.PeerService.List(1, 99999, func(tx *gorm.DB) {
		//可以公开的情报
