...
func (ct *User) Create(c *gin.Context) {
	...
	u := f.ToUser()
	err := ct.HD.Services.UserService.Create(u)
	if err != nil {
...
func (ct *User) List(c *gin.Context) {
	...
	res := ct.HD.Services.UserService.List(query.Page, query.PageSize, func(tx *gorm.DB) {
		if query.Username != "" {
...
func (ct *User) Update(c *gin.Context) {
	...
	u := f.ToUser()
	err := ct.HD.Services.UserService.Update(u)
	if err != nil {
		response.Fail(c, 101, response.TranslateMsg(c, "OperationFailed")+err.Error())
		return
	}
...
func (ct *User) Delete(c *gin.Context) {
	...
	u := ct.HD.Services.UserService.InfoById(f.Id)
	if u.Id > 0 {
		err := ct.HD.Services.UserService.Delete(u)
		if err == nil {
			response.Success(c, nil)
			return
		}
...
func (ct *User) Current(c *gin.Context) {
	...
	token, _ := c.Get("token")
	t := token.(string)
	responseLoginSuccess(c, ct.HD.Services.UserService, u, t)
}
...
func (ct *User) GroupUsers(c *gin.Context) {
	aG := ct.HD.Services.GroupService.List(1, 999, nil)
	aU := ct.HD.Services.UserService.List(1, 9999, nil)
...
func (ct *User) Register(c *gin.Context) {
	...
	regStatus := model.StatusCode(ct.HD.Config.App.RegisterStatus)
	// 注册状态可能未配置，默认启用
	if regStatus != model.COMMON_STATUS_DISABLED && regStatus != model.COMMON_STATUS_ENABLE {
		regStatus = model.COMMON_STATUS_ENABLE
	}

	u := ct.HD.Services.UserService.Register(f.Username, f.Email, f.Password, regStatus)
	if u == nil || u.Id == 0 {
