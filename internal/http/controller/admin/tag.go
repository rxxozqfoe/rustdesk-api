...
func (ct *Tag) Create(c *gin.Context) {
	...
	err := ct.HD.Services.TagService.Create(t)
	if err != nil {
...
func (ct *Tag) List(c *gin.Context) {
	...
	res := ct.HD.Services.TagService.List(query.Page, query.PageSize, func(tx *gorm.DB) {
...
func (ct *Tag) Update(c *gin.Context) {
	...
	ex := ct.HD.Services.TagService.InfoById(f.Id)
	if ex.Id == 0 {
		response.Fail(c, 101, response.TranslateMsg(c, "ItemNotFound"))
		return
	}
	t := f.ToTag()
	err := ct.HD.Services.TagService.Update(t)
	if err != nil {
		response.Fail(c, 101, response.TranslateMsg(c, "OperationFailed")+err.Error())
		return
	}
...
func (ct *Tag) Delete(c *gin.Context) {
	...
	err := ct.HD.Services.TagService.Delete(ex)
	if err == nil {
		response.Success(c, nil)
		return
	}
