...
func (s *AddressBookService) AddAddressBook(ab *model.AddressBook) error {
	return s.ctx.DB.Create(ab).Error
}
...
func (s *AddressBookService) List(page, pageSize uint, where func(tx *gorm.DB)) *model.AddressBookList {
	res := &model.AddressBookList{}
	queryList[model.AddressBook](s.ctx.DB, page, pageSize, res, &res.AddressBooks, where)
	return res
}
...
func (s *AddressBookService) SharedPeer(shareToken string) *model.ShareRecord {
	m := &model.ShareRecord{}
	s.ctx.DB.Where("share_token = ?", shareToken).First(m)
	return m
}
...
func (s *AddressBookService) ListCollection(page, pageSize uint, where func(tx *gorm.DB)) *model.AddressBookCollectionList {
	res := &model.AddressBookCollectionList{}
	queryList[model.AddressBookCollection](s.ctx.DB, page, pageSize, res, &res.AddressBookCollection, where)
	return res
}
func (s *AddressBookService) ListCollectionByIds(ids []uint) (res []*model.AddressBookCollection) {
	s.ctx.DB.Where("id in ?", ids).Find(&res)
	return res
}
...
func (s *AddressBookService) CollectionReadRules(user *model.User) (res []*model.AddressBookCollectionRule) {
	...
	tx2 := s.ctx.DB.Model(&model.AddressBookCollectionRule{})
	...
	tx3 := s.ctx.DB.Model(&model.AddressBookCollectionRule{})
...
func (s *AddressBookService) UserMaxRule(user *model.User, uid, cid uint) int {
	...
	tx := s.ctx.DB.Model(personalRules)
...
func (s *AddressBookService) RuleInfoByToIdAndCid(t int, toid, cid uint) *model.AddressBookCollectionRule {
	p := &model.AddressBookCollectionRule{}
	s.ctx.DB.Where("type = ? and to_id = ? and collection_id = ?", t, toid, cid).First(p)
	return p
}
...
func (s *AddressBookService) ListRules(page uint, size uint, f func(tx *gorm.DB)) *model.AddressBookCollectionRuleList {
	...
	queryList[model.AddressBookCollectionRule](s.ctx.DB, page, size, res, &res.AddressBookCollectionRule, f)
	return res
}
...
func (s *AddressBookService) DeleteRule(t *model.AddressBookCollectionRule) error {
	return s.ctx.DB.Delete(t).Error
}
...
func (s *AddressBookService) BatchUpdateTags(abs []*model.AddressBook, tags []string) error {
	...
	return s.ctx.DB.Model(&model.AddressBook{}).Where("row_id in ?", ids).Update("tags", tagsv).Error
}
