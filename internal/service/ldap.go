...
func (ls *LdapService) mapToLocalUser(cfg *config.Ldap, lu *LdapUser) (*model.User, error) {
	userService := ls.ctx.Services.UserService
	...
	if localUser.Id == 0 {
		...
		newUser.GroupId = 1
		if err := ls.ctx.DB.Create(newUser).Error; err != nil {
...
func (ls *LdapService) GetUserInfoByUsernameLocal(username string) (*model.User, error) {
	...
	return ls.mapToLocalUser(&ls.ctx.Config.Ldap, ldapUser)
}
