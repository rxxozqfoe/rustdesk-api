...
3. **Homepage URL**: any URL is fine
   (e.g. <https://github.com/rxxozqfoe/rustdesk-api>).
...
2. Pick the account that owns `rxxozqfoe/rustdesk-api`.
3. Choose **Only select repositories** and tick
   `rxxozqfoe/rustdesk-api`.
4. Confirm install.
...
(`INSTALLATION_ID` is not consumed by the workflow — `actions/create-github-app-token`
resolves the installation itself from `owner`/`repositories` — but it is
handy to keep noted for debugging.)

---

## 6. Point `.github/workflows/renovate.yml` at the App

The **Mint Renovate installation token** step takes a short-lived
installation token minted by the official
[`actions/create-github-app-token`](https://github.com/actions/create-github-app-token)
action from the App ID and private-key secrets above. The action
resolves the installation from `owner`/`repositories`, scopes the token
to this repo only, and pins the token's scopes in code via the
`permission-*` inputs:
